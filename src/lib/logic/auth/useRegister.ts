...
export const useRegister = (): Result<RegisterForm> => {
    const [createNewCustomer, {loading, data, error}] = useMutation<CreateNewCustomerType,
        CreateNewCustomerVars>(CREATE_NEW_CUSTOMER);
    ...
    const {values, handleChange, handleSubmit} = useForm<RegisterForm>({
        ...
        onSubmit: async (_values: CreateNewCustomerVars) => {
            try {
                console.log(_values.email, _values.password, _values.firstname, _values.lastname);
